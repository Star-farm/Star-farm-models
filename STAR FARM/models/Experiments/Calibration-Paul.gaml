...
experiment single_evaluation type: batch until: end_of_sim repeat: 1 keep_seed: true parent: generic_exp  {
	...
	init {
		...
		possible_practices <- [BAU_3S::1.0];
   		starting_date <- date([2015,1,1]) add_days (day_start_of_year -1);
   		ending_date <-  date([2024,1,1]);
   		string header <- "id,seed,rue_efficiency_factor,pest_infection_prob,pest_daily_increment,daily_water_loss_mm,max_water_capacity,lateral_leakage_coefficient,water_excess_coefficient,daily_n_consumption,toxicity_per_straw_unit,fitness\n";
